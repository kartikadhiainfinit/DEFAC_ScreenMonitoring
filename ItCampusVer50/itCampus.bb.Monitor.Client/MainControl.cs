...
namespace itCampus.bb.Monitor.Client
{
	...
	public class MainControl : IDisposable
	{
		...
		// Monitorparameter
		private MonitorState _monitorState;     // aktueller Monitorstatus
		private MonitorState _lastMonitorState; // letzter Monitorstatus
		...
		public delegate void OnDeviceConnectedEventHandler();
		public delegate void OnDeviceDisconnectedEventHandler(DeviceResult deviceResult);
		...
		// Device verbunden
		public event OnDeviceConnectedEventHandler OnDeviceConnected;

		// Device getrennt
		public event OnDeviceDisconnectedEventHandler OnDeviceDisconnected;
		...
		public void Dispose()
		{
			...
			switch (_monitor.State)
			{
				...
				case MonitorState.Assigned:
					if ((DeviceType)Settings.Instance.DeviceType == DeviceType.DialIn)
					{
		...
		/// <summary>
		/// letzter Monitorstatus
		/// </summary>
		public MonitorState LastMonitorState
		{
			get
			{
		...
		public void StopMonitoring()
		{
			...
			if ((_monitor.State != MonitorState.Started) &&
				(_monitor.State != MonitorState.Calling) &&
				(_monitor.State != MonitorState.Assigned) &&
		...
		/// <summary>
		/// baut Device auf in Abhängigkeit von aktuellem Status
		/// </summary>
		public void CallDevice(string deviceCode)
		{
			// Status prüfen
			if (_monitor.State != MonitorState.Started)
			{
			...
			// Device aufbauen
			MonitorCallDevice(deviceCode);
		}

		/// <summary>
		/// trennt Device in Abhängigkeit von aktuellem Status
		/// </summary>
		public void HangupDevice()
		{
			// Status prüfen
			if ((_monitor.State != MonitorState.Calling) &&
				(_monitor.State != MonitorState.Assigned) &&
				(_monitor.State != MonitorState.Coaching))
			{
				return;
			...
			// je nach aktuellem Status, Coaching beenden und Device trennen
			switch (_monitor.State)
			{
				case MonitorState.Coaching:
					MonitorStopCoaching();
		...
		/// baut Device auf
		/// </summary>
		private void MonitorCallDevice(string deviceCode)
		{
			// Methodenaufruf protokollieren
			Logger.Log.Info("[Monitor] <- [CallDevice] DeviceCode: " + deviceCode);

			// Device aufbauen
			_monitor.CallDevice(deviceCode);

			// Methodenabschluss protokollieren
			Logger.Log.Info("[Monitor] -> [CallDevice]");
		...
		/// trennt Device
		/// </summary>
		private void MonitorHangupDevice()
		{
			// Methodenaufruf protokollieren
			Logger.Log.Info("[Monitor] <- [HangupDevice]");

			// Device trennen
			_monitor.HangupDevice();
		...
		/// </summary>
		private void _monitor_OnDeviceConnected()
		{
			try
			{
				// Ereignis protokollieren
				Logger.Log.Info("[Monitor] -> [OnDeviceConnected]");

				// Monitorstatus aktualisieren
				RefreshMonitorState();

				// Ereignis auslösen
				RaiseOnDeviceConnected();
			}
			catch (Exception ex)
			{
				Logger.Log.Error("Error on processing event.", ex);
		...
		/// <summary>
		/// löst Ereignis OnDeviceConnected aus
		/// </summary>
		private void RaiseOnDeviceConnected()
		{
			try
			{
