...
namespace itCampus.bb.Monitor.Client
{
	...
	public class Campaign
	{
		...
		private string _trunkGroup;                 // Trunkgruppe
		...
		private int _callPriority;                  // Anrufpriorität
		...
		private int _hotlineCallPriority;           // Anrufpriorität Hotline
		...
		private int[,] _agentStateCount;            // Agentenanzahl je Modus und Status
		...
		public Campaign(itCampus.bb.Monitor.Campaign campaign, CampaignControl campaignControl)
		{
			...
			_agentStateCount = new int[3, 10];
			_agentStateTimeAvgCur = new int[10];
			...
			{
				if (frontend.CampaignSessions[_code] != null)
				{
		...
		public void Terminate()
		{
			...
			{
				frontend.UnassignCampaign(this);
			}
		...
		/// <summary>
		/// Anrufpriorität Hotline
		/// </summary>
		public int HotlineCallPriority
		{
			get
			{
				return _hotlineCallPriority;
			}
		}
		...
		private void InitializeDataCollections()
		{
			...
			_agentStateCountData.Add("Unassigned");
			...
			_agentStateTimeData.Add("Unassigned");
		...
		{
			int count = 0;
			foreach (AgentMode agentState in Enum.GetValues(typeof(AgentState)))
			{
				count += _agentStateCount[(int)agentMode, (int)agentState];
			}
			return count;
		}

		/// <summary>
		/// gibt Anzahl aktiver Agenten für angegebenen Modus zurück
		/// </summary>
		public int GetAgentModeCountActive(AgentMode agentMode)
		{
			return GetAgentModeCount(agentMode) -
				_agentStateCount[(int)agentMode, (int)AgentState.Unassigned];
		}

		/// <summary>
		/// gibt Anzahl inaktiver Agenten für angegebenen Modus zurück
		/// </summary>
		public int GetAgentModeCountInactive(AgentMode agentMode)
		{
			return _agentStateCount[(int)agentMode, (int)AgentState.Unassigned];
		}
		...
		public void RefreshParameters(itCampus.bb.Monitor.Campaign campaign)
		{
			...
			_trunkGroup = campaign.TrunkGroup;
			...
			_callPriority = campaign.CallPriority;
			...
			_hotlineCallPriority = campaign.HotlineCallPriority;
			...
			foreach (AgentMode agentMode in Enum.GetValues(typeof(AgentMode)))
			{
				...
				{
					_agentStateCount[(int)agentMode, (int)agentState] =
						campaign.GetAgentStateCount(agentMode, agentState);
				}
			}
			foreach (AgentState agentState in Enum.GetValues(typeof(AgentState)))
			{
			...
			_agentCount = 0;
			foreach (int count in _agentStateCount)
			{
				_agentCount += count;
			}
			_callJobCount = 0;
		...
		private void RefreshDataCollections(itCampus.bb.Monitor.Campaign campaign)
		{
			...
			_agentStateCountData["Unassigned"].Value =
				GetAgentStateCount(AgentState.Unassigned);
			_agentStateCountData["NotReady"].Value =
				GetAgentStateCount(AgentState.NotReady);
			_agentStateCountData["Ready"].Value =
				GetAgentStateCount(AgentState.Ready);
			_agentStateCountData["Pending"].Value =
				GetAgentStateCount(AgentState.Pending);
			_agentStateCountData["Signaling"].Value =
				GetAgentStateCount(AgentState.Signaling);
			_agentStateCountData["Preparing"].Value =
				GetAgentStateCount(AgentState.Preparing);
			_agentStateCountData["Calling"].Value =
				GetAgentStateCount(AgentState.Calling);
			_agentStateCountData["Talking"].Value =
				GetAgentStateCount(AgentState.Talking);
			_agentStateCountData["Conferencing"].Value =
				GetAgentStateCount(AgentState.Conferencing);
			_agentStateCountData["Wrapup"].Value =
				GetAgentStateCount(AgentState.Wrapup);
			_agentStateCountData["Total"].Value = _agentCount;

			// Verlaufsdaten Agentenzeiten aktualisieren
			_agentStateTimeData["Unassigned"].Value =
				campaign.GetAgentStateTimeAvgCur(AgentState.Unassigned);
